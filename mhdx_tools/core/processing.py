...
import glob
import pickle as pk
from mhdx_tools.core import io, datatypes
...
def create_factor_data_object(data_tensor, gauss_params, timepoint_label=None):
    """Stores DataTensor and subsequent Factor object attributes to dictionary.
        ...
        Args:
            data_tensor (DataTensor): mhdx_tools.core.datatypes.DataTensor that has run the factorize() function.
...
class PathOptimizer:
    ...
    def delta_mz_rate(self, ics):
        ...
        for i in range(2, len(ics)):
            ...
            if round(ics[i].timepoint_idx - ics[i - 1].timepoint_idx, 2) <= 0.1:
                continue
            current_rate = max([
                (new_com - ics[i - 1].baseline_integrated_mz_com), 0.1
            ]) / (ics[i].timepoint_idx - ics[i -1].timepoint_idx)
